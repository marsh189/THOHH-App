Shader "Custom/AlphaUnlitMaskColor_Clipping" {	
	Properties {		
		_MainTex ("Pattern", 2D) = "white" {}	
		_ClippingMask ("ClippingMask", 2D) = "white" {}
		_RegionTex ("Mask (Alpha8)", 2D) = "white" {}		
...
#pragma exclude_renderers d3d11 xbox360
     ...
     v2f vert (appdata_full v)
     {
	     ...
	     o.uv2 = TRANSFORM_TEX (v.texcoord1, _RegionTex);
	     
	     o.color =  v.color;
	     o.originMaskColor = tex2D(_RegionTex, float2(_OriginX, _OriginY));
	     return o;
     }
     ...
     half4 frag (v2f i) : COLOR
     {
         ...
         result.rgb *= result.a;
         fixed4 mask = tex2D(_RegionTex, i.uv2);
